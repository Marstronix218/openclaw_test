...
echo
echo "=== model server ==="
docker exec "$CONTAINER" bash -lc "curl -s --max-time 3 http://127.0.0.1:${MODEL_PORT}/health 2>/dev/null || echo 'not responding'; echo; echo '--- server.log (tail) ---'; tail -n 10 /root/model-server/server.log 2>/dev/null || true" 2>/dev/null || true
echo
echo "=== gateway url ==="
